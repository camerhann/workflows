...
- `commit.pr_url`
- `commit.issue_closed`
- `commit.test_instructions` - How to test locally
...
# Visit http://localhost:3000 and navigate to the affected page
\`\`\`
...
4. Issue #${ISSUE_NUMBER} will be closed automatically
...
Based on the files changed, generate appropriate local testing instructions:

```bash
# Detect project type and generate test command
CHANGED_FILES=$(cat issue-state.json | jq -r '.writer.changes[].file')

# Determine which part of the app to test
if echo "$CHANGED_FILES" | grep -q "frontend/"; then
  TEST_CMD="cd frontend && npm run dev"
  TEST_URL="http://localhost:3000"
elif echo "$CHANGED_FILES" | grep -q "backend/"; then
  TEST_CMD="cd backend && uvicorn app.main:app --reload"
...
# Determine which page to visit based on file path
# e.g., forecast/snapshot-cards.tsx → /forecast or /dashboard
```

Include these instructions in:
1. The PR body
2. The issue comment
3. The summary output

### 9. Close the Issue (Optional)
...
```json
{
  "commit": {
    ...
    "test_instructions": {
      ...
      "url": "http://localhost:3000/allocations",
      "steps": ["Navigate to Allocations page", "Verify slider behavior"]
    }
  },
  "pipeline": {
    "status": "completed"
